//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UIView.h>

@class UIImageView, UILabel;

@interface OPROnlineUserErrorView : UIView
{
    CDUnknownBlockType _retyrHandler;
    UILabel *_tips;
    UIImageView *_emptyImageView;
}

@property(retain, nonatomic) UIImageView *emptyImageView; // @synthesize emptyImageView=_emptyImageView;
@property(retain, nonatomic) UILabel *tips; // @synthesize tips=_tips;
@property(copy, nonatomic) CDUnknownBlockType retyrHandler; // @synthesize retyrHandler=_retyrHandler;
- (void).cxx_destruct;
- (void)onRetryClick;
- (void)layoutSubviews;
- (id)initWithFrame:(struct CGRect)arg1;

@end

